...
public:
    ...
    bool EditValue(const std::string& key, const std::string& newValue)
    {
        ...
        if (it != value.end()) { it->second = newValue; return true; }
    ...
    bool ResetValue(const std::string& key)
    {
        ...
        if (def != defaultValues.end()) { value[key] = def->second; return true; }
    ...
    void PrintValues() const
    {
        ...
        for (const auto& [k, v] : value) std::cout << "  " << k << " = \"" << v << "\"\n";
    }
    ...
    std::map<std::string, std::string> GetValues(const std::wstring& gdtAbsPath) const
    {
        ...
        while (c.ok()) {
            ...
            const char* save = c.p;
            if (!scan_quoted(c, svName)) { ++c.p; continue; }

            // must match asset name
            if (!iequals(svName, Name)) { c.p = save + 1; continue; }

            skip_ws(c);
            if (!scan_char(c, '(')) { c.p = save + 1; continue; }
            skip_ws(c);
            if (!scan_quoted(c, svGdf)) { c.p = save + 1; continue; }
            skip_ws(c);
            if (!scan_char(c, ')')) { c.p = save + 1; continue; }
            skip_ws(c);
...
private:
    ...
    static bool scan_ident(Cursor& c, std::string& out)
    {
        ...
        const char* beg = c.p;
        unsigned char ch = (unsigned char)*c.p;
        if (!(std::isalpha(ch) || ch == '_' || ch == '$')) return false;
        ++c.p;
        while (c.ok()) {
    ...
    static void parse_body_into_map(Cursor& c, std::map<std::string, std::string>& out)
    {
        ...
        while (c.ok() && depth > 0) {
            ...
            if (scan_quoted(c, valQ)) {
                ...
                const char* end = c.p;
                while (end > beg && (unsigned char)*(end - 1) <= ' ') --end;
    ...
    static int SchemaCB(void* ctx, int argc, char** argv, char**)
    {
        ...
        if (name == "PK_id" || (!name.empty() && name[0] == '_')) return 0;
    ...
    static std::wstring getToolsRootW() {
        ...
        if (_wdupenv_s(&val, &len, L"TA_TOOLS_PATH") != 0 || !val) return {};
        std::wstring root(val); free(val);
